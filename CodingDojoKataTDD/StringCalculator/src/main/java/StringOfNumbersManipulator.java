...
public class StringOfNumbersManipulator {
    ...
    public boolean isIllegalSeparator(){
    ...
    private boolean searchForIllegalSeparators(String pattern){
        Pattern p = Pattern.compile(pattern);
        Matcher m = p.matcher(numbers);
        if (m.find()){
            illegalSeparatorPosition = m.start() +1;
            return true;
        }
        return false;
    }
